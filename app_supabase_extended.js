...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // Adicionando a função de fechar o modal de operadores que estava faltando
  const closeOperatorsModal = () => $('#operators-modal').setAttribute('aria-hidden','true');
  // Você precisará adicionar um botão no HTML para fechar este modal com id="close-operators-modal"
  // Exemplo: <button type="button" id="close-operators-modal">Fechar</button>
...
// OPERATOR (signup)
async function createOperator(e){ e.preventDefault(); const email=$('#op-email').value, pass=$('#op-pass').value; const res = await supabaseClient.auth.signUp({email, password: pass}); if(res.error){ alert('Erro criar operador: '+res.error.message); return; } alert('Operador criado: '+email); $('#op-email').value=''; $('#op-pass').value=''; }
...
// PRODUCTS & CART (basic)
async function loadProducts(){ const res = await supabaseClient.from('products').select('*').order('name'); if(res.error){ console.error(res.error); window._POS_PRODUCTS=[]; return; } window._POS_PRODUCTS = res.data || []; renderProducts(); }
...
async function completeSale(){ if(cart.length===0){ alert('Carrinho vazio'); return; } const totals = computeTotals(); const paid = parseFloat($('#amount-paid').value || '0'); if(isNaN(paid) || paid < totals.total){ if(!confirm('Valor recebido menor que total. Registrar mesmo assim?')) return; } for(const it of cart){ const p=(window._POS_PRODUCTS||[]).find(x=>x.id===it.id); if(!p){ alert('Produto ausente: '+it.name); return; } if(p.qty < it.qty){ alert('Estoque insuficiente: '+it.name); return; } } for(const it of cart){ const p=(window._POS_PRODUCTS||[]).find(x=>x.id===it.id); const newQty = p.qty - it.qty; const res = await supabaseClient.from('products').update({qty:newQty}).eq('id', p.id); if(res.error){ alert('Erro atualizar estoque: '+res.error.message); return; } } const user = (await supabaseClient.auth.getUser()).data?.user; const sale = { id: 's_' + Date.now(), datetime: new Date().toISOString(), items: cart, totals, paid, operator: user?.email || 'anon' }; const insert = await supabaseClient.from('sales').insert({id: sale.id, datetime: sale.datetime, payload: sale}); if(insert.error){ alert('Erro salvar venda: '+insert.error.message); return; } await loadProducts(); await loadCashflow(); alert('Venda registrada.'); generateReceipt(sale); cart=[]; renderCart(); $('#amount-paid').value=''; await refreshDashboard(); }

// load cashflow
async function loadCashflow(){ const res = await supabaseClient.from('sales').select('id', 'datetime', 'payload').order('datetime', {ascending:false}).limit(500); if(res.error){ console.error(res.error); window._POS_CASH=[]; return; } window._POS_CASH = res.data.map(r=> r.payload || r); renderCashflow(); }
...
// dashboard
async function refreshDashboard(){ const res = await supabaseClient.from('sales').select('payload').order('datetime', {ascending:false}).limit(2000); if(res.error){ console.error(res.error); return; } const rows = (res.data || []).map(r=> r.payload || r); const now = new Date(); const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate()); const startOfWeek = new Date(startOfToday.getTime() - (startOfToday.getDay()*24*60*60*1000)); const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1); const startOfQuarter = new Date(now.getFullYear(), Math.floor(now.getMonth()/3)*3, 1); const startOfYear = new Date(now.getFullYear(), 0, 1); const sum = l => l.reduce((s,i)=> s + (i.totals?.total || 0), 0); const today = rows.filter(r=> new Date(r.datetime) >= startOfToday); const week = rows.filter(r=> new Date(r.datetime) >= startOfWeek); const month = rows.filter(r=> new Date(r.datetime) >= startOfMonth); const quarter = rows.filter(r=> new Date(r.datetime) >= startOfQuarter); const year = rows.filter(r=> new Date(r.datetime) >= startOfYear); document.getElementById('metric-today').textContent = formatBR(sum(today)); document.getElementById('metric-week').textContent = formatBR(sum(week)); document.getElementById('metric-month').textContent = formatBR(sum(month)); document.getElementById('metric-quarter').textContent = formatBR(sum(quarter)); document.getElementById('metric-year').textContent = formatBR(sum(year)); }

// cash closing
async function closeDay(){ if(!confirm('Confirma fechamento do caixa do dia?')) return; const start = new Date(); start.setHours(0,0,0,0); const res = await supabaseClient.from('sales').select('payload').gte('datetime', start.toISOString()); if(res.error){ alert('Erro obter vendas: '+res.error.message); return; } const rows = (res.data||[]).map(r=> r.payload || r); const totalToday = rows.reduce((s,i)=> s + (i.totals?.total || 0), 0); const closure = { id: 'c_' + Date.now(), date: new Date().toISOString(), total: totalToday, details: rows }; const up = await supabaseClient.from('cash_closures').insert(closure); if(up.error){ alert('Erro salvar fechamento: '+up.error.message); return; } document.getElementById('close-result').textContent = 'Fechamento registrado: ' + formatBR(totalToday); await refreshDashboard(); }
